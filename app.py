...
# Queue for handling updates safely
...
def run_bot_worker():
    """Runs the bot's event loop in a separate thread."""
    asyncio.run(_bot_worker())

async def _bot_worker():
    """Internal worker that processes updates from the queue."""
    ...
    print("🚀 Bot worker started. Waiting for updates...")
...
def telegram_webhook():
    """Receives updates from Telegram and puts them in the queue."""
    try:
...
if __name__ == '__main__':
    ...
    bot_thread = threading.Thread(target=run_bot_worker)
    ...
    try:
        # We need a temporary loop just to set the webhook
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            application_temp = Application.builder().token(TOKEN).build()
            loop.run_until_complete(application_temp.bot.set_webhook(webhook_url))
