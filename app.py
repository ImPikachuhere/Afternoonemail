...
def parse_line(line):
    ...
    try:
        # Find the last two colons to separate URL, User, and Pass
...
async def process_file_task(chat_id, file_path, context):
    ...
    try:
        ...
        IS_PROCESSING = False
        await context.bot.send_message(chat_id=chat_id, text="❌ Error reading file. Ensure it is a valid text file.")
    ...
    connector = aiohttp.TCPConnector(limit=20) # Limit concurrency
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for line in lines:
            ...
            # Send update every 5% or if it's the last item
    ...
    if valid_results:
        ...
        await context.bot.send_message(
            ...
            text=f"🛑 **Process Complete**\n\n"
                 ...
                 f"No valid credentials were found in the provided list.",
            parse_mode='Markdown'
        )
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "- Sites with CAPTCHA/2FA cannot be checked.",
        parse_mode='Markdown'
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "2. Format must be `URL:username:password` or `URL:email:password`.\n"
        "3. Send the file to me.\n"
        "4. Wait for the progress updates.\n"
        "5. Download the result file containing only valid credentials.\n\n"
        "If you encounter errors, ensure your file format is correct."
    )
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if IS_PROCESSING:
        await update.message.reply_text(
            "⚠️ **Busy!**\n\n"
            "I am currently processing another file. Please wait for the current job to finish before sending a new one.\n\n"
            "This prevents server crashes and ensures accurate results.",
            parse_mode='Markdown'
...
if __name__ == '__main__':
    ...
    application.add_handler(MessageHandler(filters.TEXT, start_command)) # Treat unknown text as help/start
