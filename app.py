...
def run_bot_loop():
    ...
    try:
        ...
        # We use run_polling with a very long timeout or just let it run
        # But for webhook mode, we just need the loop to be running to process updates
        # The actual processing is triggered by the webhook handler via run_coroutine_threadsafe
...
if __name__ == '__main__':
    # Removed 'global bot_loop, application' from here because this is the global scope
    # and we are assigning to these variables, which is valid without 'global' in this context.
    # Declaring 'global' here while also assigning causes the SyntaxError.
    ...
    try:
        # We need a temporary loop to set the webhook since the main loop is busy
