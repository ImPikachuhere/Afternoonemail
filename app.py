...
def telegram_webhook():
    """
    Receives updates from Telegram and passes them to the application for processing.
    """
    try:
        ...
        # We use asyncio.run() here because this function is synchronous (Flask view)
        # but process_update is async. This is safe because we are not inside an existing event loop.
        # HOWEVER, running a full loop for every request is inefficient. 
        # The correct way with PTB v20+ and Flask is to use the application's internal queue.
        
        # CORRECT APPROACH FOR PTB v20+:
        # We must schedule the processing on the application's running loop.
        # Since we started the application's loop in a separate thread (see below), 
        # we need to submit the task to that specific loop.
        
        loop = asyncio.get_event_loop()
        if loop.is_running():
            # Schedule the coroutine on the running loop
            future = asyncio.run_coroutine_threadsafe(application.process_update(update), loop)
            # We don't wait for the result, just let it run in the background
            return 'OK', 200
        else:
            print("Error: Event loop is not running. Bot may not function.")
...
def run_bot_loop():
    ...
    # This starts the application's internal processing loop
    # It won't return until the application is stopped
    try:
        application.run_webhook(
            listen="0.0.0.0",
            port=PORT + 1, # Use a different port for internal webhook if needed, but we are handling manually
            url_token=TOKEN, # This is not used in manual mode, but required by signature
            allowed_updates=Update.ALL_TYPES,
            drop_pending_updates=False
        )
    except Exception as e:
        print(f"Error in run_webhook: {e}")
...
if __name__ == '__main__':
    ...
    # This is required because Flask blocks the main thread
    ...
    try:
        # We need a temporary loop to set the webhook
